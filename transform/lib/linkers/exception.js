...
export class ExceptionLinker extends Visitor {
    ...
    visitCallExpression(node, ref = null) {
        ...
        if (fnName == "abort" || fnName == "unreachable") {
            ...
            if (linked.imported && !linked.path) {
                ...
                if (DEBUG)
                    console.log("Import (call): " + toString(importStmt) + " at " + node.range.source.internalPath);
            }
    ...
    visitThrowStatement(node, ref) {
        ...
        if (value.kind != 17 ||
            value.typeName.identifier.text != "Error")
...
function calcPath(from, toName) {
    ...
    let relPath = path.posix
        .join(...path
        .relative(path.dirname(from), path.join(baseDir, "assembly", "types", toName))
        .split(path.sep))
